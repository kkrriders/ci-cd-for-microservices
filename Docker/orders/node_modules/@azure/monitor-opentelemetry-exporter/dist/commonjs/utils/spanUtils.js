...
function createTagsFromSpan(span) {
    ...
    const httpUserAgent = span.attributes[semantic_conventions_1.SEMATTRS_HTTP_USER_AGENT];
    if (httpUserAgent) {
    ...
    if (span.kind === api_1.SpanKind.SERVER) {
        ...
        const httpClientIp = span.attributes[semantic_conventions_1.SEMATTRS_HTTP_CLIENT_IP];
        const netPeerIp = span.attributes[semantic_conventions_1.SEMATTRS_NET_PEER_IP];
        if (httpMethod) {
            const httpRoute = span.attributes[semantic_conventions_1.SEMATTRS_HTTP_ROUTE];
    ...
    if (attributes) {
        for (const key of Object.keys(attributes)) {
            ...
            if (!(key.startsWith("_MS.") ||
                ...
                key === semantic_conventions_1.SEMATTRS_NET_PEER_IP ||
                key === semantic_conventions_1.SEMATTRS_NET_PEER_NAME ||
...
function createDependencyData(span) {
    ...
    if (httpMethod) {
        ...
        const httpStatusCode = span.attributes[semantic_conventions_1.SEMATTRS_HTTP_STATUS_CODE];
        if (httpStatusCode) {
...
function createRequestData(span) {
    const requestData = {
        ...
        success: span.status.code !== api_1.SpanStatusCode.ERROR &&
            (Number(span.attributes[semantic_conventions_1.SEMATTRS_HTTP_STATUS_CODE]) || 0) < 400,
        responseCode: "0",
        duration: (0, breezeUtils_js_1.msToTimeSpan)((0, core_1.hrTimeToMilliseconds)(span.duration)),
        version: 2,
        source: undefined,
        ...
        span.events.forEach((event) => {
            ...
            if (event.name === "exception") {
                ...
                if (parentSpanContext) {
                    isValidParent =
                        (0, api_1.isValidTraceId)(parentSpanContext.traceId) && (0, api_1.isValidSpanId)(parentSpanContext.spanId);
                }
                /*
                 ...
                 */
                if (!isValidParent || (parentSpanContext === null || parentSpanContext === void 0 ? void 0 : parentSpanContext.isRemote) || span.kind === api_1.SpanKind.INTERNAL) {
