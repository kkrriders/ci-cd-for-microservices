...
function getUrl(attributes) {
    ...
    const httpMethod = attributes[semantic_conventions_1.SEMATTRS_HTTP_METHOD];
    if (httpMethod) {
        const httpUrl = attributes[semantic_conventions_1.SEMATTRS_HTTP_URL];
        if (httpUrl) {
            return String(httpUrl);
        }
        else {
            const httpScheme = attributes[semantic_conventions_1.SEMATTRS_HTTP_SCHEME];
            const httpTarget = attributes[semantic_conventions_1.SEMATTRS_HTTP_TARGET];
            if (httpScheme && httpTarget) {
                const httpHost = attributes[semantic_conventions_1.SEMATTRS_HTTP_HOST];
                if (httpHost) {
                    return `${httpScheme}://${httpHost}${httpTarget}`;
                }
                else {
                    const netPeerPort = attributes[semantic_conventions_1.SEMATTRS_NET_PEER_PORT];
                    if (netPeerPort) {
                        const netPeerName = attributes[semantic_conventions_1.SEMATTRS_NET_PEER_NAME];
                        if (netPeerName) {
                            return `${httpScheme}://${netPeerName}:${netPeerPort}${httpTarget}`;
                        }
                        else {
                            const netPeerIp = attributes[semantic_conventions_1.SEMATTRS_NET_PEER_IP];
                            if (netPeerIp) {
