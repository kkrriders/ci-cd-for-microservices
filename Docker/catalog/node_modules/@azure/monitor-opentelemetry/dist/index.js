...
function parseResourceDetectorsFromEnvVar() {
    ...
    if (process.env.OTEL_NODE_RESOURCE_DETECTORS != null) {
        const resourceDetectorsFromEnv = (_b = (_a = process.env.OTEL_NODE_RESOURCE_DETECTORS) === null || _a === undefined ? undefined : _a.split(",")) !== null && _b !== undefined ? _b : [
            "env",
            "host",
            "os",
        ];
...
class InternalConfig {
    ...
    _setDefaultResource() {
        ...
        if (vmResource.asyncAttributesPending) {
            (_a = vmResource.waitForAsyncAttributes) === null || _a === undefined ? undefined : _a.call(vmResource).then(() => {
...
function getDependencyTarget(attributes) {
    ...
    const hostPort = attributes[semanticConventions.SEMATTRS_NET_HOST_PORT];
...
class StandardMetrics {
    ...
    constructor(config, options) {
        ...
        const metricReaderOptions = {
            ...
            exportIntervalMillis: (options === null || options === undefined ? undefined : options.collectionInterval) || this._collectionInterval,
        };
...
class QuickpulseClient extends coreClient__namespace.ServiceClient {
    ...
    constructor(options) {
        ...
        const optionsWithDefaults = Object.assign(Object.assign(Object.assign({}, defaults), options), { userAgentOptions: {
                userAgentPrefix,
            }, endpoint: (_b = (_a = options.endpoint) !== null && _a !== undefined ? _a : options.baseUri) !== null && _b !== undefined ? _b : "{endpoint}" });
...
const AZURE_MONITOR_OPENTELEMETRY_VERSION = "1.8.1";
...
function getSdkVersion() {
    ...
    const internalSdkVersion = `${(_a = process.env[AZURE_MONITOR_PREFIX]) !== null && _a !== undefined ? _a : ""}node${nodeVersion}:otel${opentelemetryVersion}:${version}`;
...
class LiveMetrics {
    ...
    shutdown() {
        var _a;
        (_a = this.meterProvider) === null || _a === undefined ? undefined : _a.shutdown();
    }
    async goQuickpulse() {
    ...
    activateMetrics(options) {
        ...
        const metricReaderOptions = {
            ...
            exportIntervalMillis: options === null || options === undefined ? undefined : options.collectionInterval,
        };
    ...
    async flush() {
        var _a;
        await ((_a = this.meterProvider) === null || _a === undefined ? undefined : _a.forceFlush());
    }
    /**
     *Get OpenTelemetry MeterProvider
     */
...
class MetricHandler {
    ...
    constructor(config, options) {
        ...
        this._views = [];
        if ((_a = config.instrumentationOptions.azureSdk) === null || _a === undefined ? undefined : _a.enabled) {
            this._views.push(new sdkMetrics.View({ meterName: "@azure/opentelemetry-instrumentation-azure-sdk" }));
        }
        if ((_b = config.instrumentationOptions.http) === null || _b === undefined ? undefined : _b.enabled) {
            this._views.push(new sdkMetrics.View({ meterName: "@azure/opentelemetry-instrumentation-http" }));
        }
        if ((_c = config.instrumentationOptions.mongoDb) === null || _c === undefined ? undefined : _c.enabled) {
            this._views.push(new sdkMetrics.View({ meterName: "@azure/opentelemetry-instrumentation-mongodb" }));
        }
        if ((_d = config.instrumentationOptions.mySql) === null || _d === undefined ? undefined : _d.enabled) {
            this._views.push(new sdkMetrics.View({ meterName: "@opentelemetry/instrumentation-mysql" }));
        }
        if ((_e = config.instrumentationOptions.postgreSql) === null || _e === undefined ? undefined : _e.enabled) {
            this._views.push(new sdkMetrics.View({ meterName: "@opentelemetry/instrumentation-pg" }));
        }
        if ((_f = config.instrumentationOptions.redis4) === null || _f === undefined ? undefined : _f.enabled) {
            this._views.push(new sdkMetrics.View({ meterName: "@opentelemetry/instrumentation-redis-4" }));
        }
        if ((_g = config.instrumentationOptions.redis) === null || _g === undefined ? undefined : _g.enabled) {
    ...
    markSpanAsProcessed(span) {
        var _a;
        (_a = this._standardMetrics) === null || _a === undefined ? undefined : _a.markSpanAsProcessed(span);
    }
    recordSpan(span) {
        var _a, _b;
        (_a = this._standardMetrics) === null || _a === undefined ? undefined : _a.recordSpan(span);
        (_b = this._liveMetrics) === null || _b === undefined ? undefined : _b.recordSpan(span);
    }
    recordLog(logRecord) {
        var _a, _b;
        (_a = this._standardMetrics) === null || _a === undefined ? undefined : _a.recordLog(logRecord);
        (_b = this._liveMetrics) === null || _b === undefined ? undefined : _b.recordLog(logRecord);
    }
    /**
     * Shutdown handler
     */
    ...
    async shutdown() {
        var _a, _b;
        (_a = this._standardMetrics) === null || _a === undefined ? undefined : _a.shutdown();
        (_b = this._liveMetrics) === null || _b === undefined ? undefined : _b.shutdown();
    }
}
...
class TraceHandler {
    ...
    _initializeInstrumentations() {
        var _a, _b, _c, _d, _e, _f, _g;
        if ((_a = this._config.instrumentationOptions.http) === null || _a === undefined ? undefined : _a.enabled) {
        ...
        if ((_b = this._config.instrumentationOptions.azureSdk) === null || _b === undefined ? undefined : _b.enabled) {
        ...
        if ((_c = this._config.instrumentationOptions.mongoDb) === null || _c === undefined ? undefined : _c.enabled) {
        ...
        if ((_d = this._config.instrumentationOptions.mySql) === null || _d === undefined ? undefined : _d.enabled) {
            this._instrumentations.push(new instrumentationMysql.MySQLInstrumentation(this._config.instrumentationOptions.mySql));
        }
        if ((_e = this._config.instrumentationOptions.postgreSql) === null || _e === undefined ? undefined : _e.enabled) {
            this._instrumentations.push(new instrumentationPg.PgInstrumentation(this._config.instrumentationOptions.postgreSql));
        }
        if ((_f = this._config.instrumentationOptions.redis) === null || _f === undefined ? undefined : _f.enabled) {
            this._instrumentations.push(new instrumentationRedis.RedisInstrumentation(this._config.instrumentationOptions.redis));
        }
        if ((_g = this._config.instrumentationOptions.redis4) === null || _g === undefined ? undefined : _g.enabled) {
...
class LogHandler {
    ...
    _initializeInstrumentations() {
        ...
        if ((_a = this._config.instrumentationOptions.bunyan) === null || _a === undefined ? undefined : _a.enabled) {
        ...
        if ((_b = this._config.instrumentationOptions.winston) === null || _b === undefined ? undefined : _b.enabled) {
...
class BrowserSdkLoader {
    constructor(config) {
        ...
        let clientWebIkey;
        if ((_a = config.browserSdkLoaderOptions) === null || _a === undefined ? undefined : _a.connectionString) {
            clientWebIkey = this._getBrowserSdkLoaderIkey((_b = config === null || config === undefined ? undefined : config.browserSdkLoaderOptions) === null || _b === undefined ? undefined : _b.connectionString);
        }
...
function useAzureMonitor(options) {
    ...
    const statsbeatInstrumentations = {
        // Instrumentations
        azureSdk: (_b = (_a = config.instrumentationOptions) === null || _a === undefined ? undefined : _a.azureSdk) === null || _b === undefined ? undefined : _b.enabled,
        mongoDb: (_d = (_c = config.instrumentationOptions) === null || _c === undefined ? undefined : _c.mongoDb) === null || _d === undefined ? undefined : _d.enabled,
        mySql: (_f = (_e = config.instrumentationOptions) === null || _e === undefined ? undefined : _e.mySql) === null || _f === undefined ? undefined : _f.enabled,
        postgreSql: (_h = (_g = config.instrumentationOptions) === null || _g === undefined ? undefined : _g.postgreSql) === null || _h === undefined ? undefined : _h.enabled,
        redis: (_k = (_j = config.instrumentationOptions) === null || _j === undefined ? undefined : _j.redis) === null || _k === undefined ? undefined : _k.enabled,
        bunyan: (_m = (_l = config.instrumentationOptions) === null || _l === undefined ? undefined : _l.bunyan) === null || _m === undefined ? undefined : _m.enabled,
        winston: (_p = (_o = config.instrumentationOptions) === null || _o === undefined ? undefined : _o.winston) === null || _p === undefined ? undefined : _p.enabled,
    };
    const statsbeatFeatures = {
        browserSdkLoader: config.browserSdkLoaderOptions.enabled,
        aadHandling: !!((_q = config.azureMonitorExporterOptions) === null || _q === undefined ? undefined : _q.credential),
        diskRetry: !((_r = config.azureMonitorExporterOptions) === null || _r === undefined ? undefined : _r.disableOfflineStorage),
    };
    ...
    const spanProcessors = (options === null || options === undefined ? undefined : options.spanProcessors) || [];
    const logRecordProcessors = (options === null || options === undefined ? undefined : options.logRecordProcessors) || [];
...
function shutdownAzureMonitor() {
    browserSdkLoader === null || browserSdkLoader === undefined ? undefined : browserSdkLoader.dispose();
    return sdk === null || sdk === undefined ? undefined : sdk.shutdown();
}
